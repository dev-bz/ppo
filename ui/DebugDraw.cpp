...
Camera g_camera;
b2Draw *debugDraw = &g_debugDraw;
extern "C" void debugCreate(int w, int h) {
  g_debugDraw.Create();
  g_camera.m_width = w;
  g_camera.m_height = h;
  g_camera.m_zoom = 0.85;
  g_camera.m_center.y = 8.0;
  if (w > h) {
    g_camera.m_zoom = 0.3;
    g_camera.m_center.y = 5.0;
  }
  const char *fontPath = "/system/fonts/Miui-Regular.ttf"; // VeraMono.ttf";
  int test = 0;
...
struct GLRenderMeshes {
  void Create() {
    ...
    for (int y = 0; y < 32; ++y) {
      for (int x = 0; x < 64; ++x) {
        m_vertices[y * 64 + x] = 0.25 * b2Vec2(x - 31.5, 15.5 - y);
        m_colors[y * 64 + x] =
            b2Color(drand48() * 0.25f, drand48() * 0.25f, drand48() * 0.25f);
      }
    }
    int i = 0;
    for (unsigned short y = 0; y < 31; ++y) {
      for (unsigned short x = 0; x < 63; ++x) {
        if (x != 31) {
          unsigned short a = y * 64 + x;
          unsigned short b = a + 1;
          unsigned short c = a + 64;
  ...
  void Vertex(const b2Vec2 &v, bool value, const b2Color &c) {
    // if (m_count == e_maxVertices) Flush();
    static int index = 0;
    static b2Vec2 half(0.5, 0.5);
    if (value) {
      int tx = 16 + (v.x * 15.5);
      int ty = 16 - (v.y * 15.5);
      index = ty * 64 + tx;
    ...
    if (index >= 0 && index < e_maxVertices) {
      // m_vertices[index] = v;
      m_colors[index] = c;
    }
    //++m_count;
  ...
  void Flush() {
    ...
    glBindVertexArray(m_vaoId);
    glBindBuffer(GL_ARRAY_BUFFER, m_vboIds[0]);
    glBufferSubData(GL_ARRAY_BUFFER, 0, e_maxVertices * sizeof(b2Vec2),
                    m_vertices);
    glVertexAttribPointer(m_vertexAttribute, 2, GL_FLOAT, GL_FALSE, 0, 0);
    glBindBuffer(GL_ARRAY_BUFFER, m_vboIds[1]);
    glBufferSubData(GL_ARRAY_BUFFER, 0, e_maxVertices * sizeof(b2Color),
                    m_colors);
    glVertexAttribPointer(m_colorAttribute, 4, GL_FLOAT, GL_FALSE, 0, 0);
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    glDrawElements(GL_TRIANGLES, 62 * 31 * 6, GL_UNSIGNED_SHORT, indices);
    // glDrawArrays(GL_POINTS, 0, e_maxVertices);
    glDisable(GL_BLEND);
    sCheckGLError();
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    glBindVertexArray(0);
  ...
  enum { e_maxVertices = 64 * 32 };
  b2Vec2 m_vertices[e_maxVertices];
  b2Color m_colors[e_maxVertices];
  unsigned short indices[62 * 31 * 6];
  // int32 m_count;
  b2Vec2 left, right;
  GLuint m_vaoId;
  GLuint m_vboIds[2];
...
void DebugDraw::DrawString(int x, int y, const char *string, ...) {
  ...
  va_end(arg);
  AddGfxCmdText(float(x), h - float(y), TEXT_ALIGN_LEFT, buffer,
                SetRGBA(230, 153, 153, 255));
}
void DebugDraw::DrawString(const b2Vec2 &pw, const char *string, ...) {
  ...
  vsprintf(buffer, string, arg);
  va_end(arg);
  AddGfxCmdText(ps.x, h - ps.y, TEXT_ALIGN_LEFT, buffer,
                SetRGBA(230, 153, 153, 255));
}
...
extern "C" void setValuesColor(const b2Vec2 &v, const b2Color &c) {
  // g_debugDraw.m_meshes->Vertex(v, true, c);
}