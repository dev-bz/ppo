...
struct GLRenderMeshes {
	...
	glBindAttribLocation(m_programId, 1, "v_color");
		...
		for (int y = 0; y < 32; ++y) {
			for (int x = 0; x < 64; ++x) {
				...
				m_colors[y * 64 + x] = b2Color(drand48() * 0.25f, drand48() * 0.25f, drand48() * 0.25f);
			}
		}
		int i = 0;
	...
	void Flush() {
		...
		glBindVertexArray(m_vaoId);
		glBindBuffer(GL_ARRAY_BUFFER, m_vboIds[0]);
		glBufferSubData(GL_ARRAY_BUFFER, 0, e_maxVertices * sizeof(b2Vec2), m_vertices);
		glVertexAttribPointer(m_vertexAttribute, 2, GL_FLOAT, GL_FALSE, 0, 0);
		glBindBuffer(GL_ARRAY_BUFFER, m_vboIds[1]);
		glBufferSubData(GL_ARRAY_BUFFER, 0, e_maxVertices * sizeof(b2Color), m_colors);
		glVertexAttribPointer(m_colorAttribute, 4, GL_FLOAT, GL_FALSE, 0, 0);
		glEnable(GL_BLEND);
		glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
