...
// Json keys
const std::string gJointTypeNames[cKinTree::eJointTypeMax] = {"revolute", "planar",		 "prismatic",
																															"fixed",		"spherical", "none"};
const std::string gJointsKey = "Joints";
const std::string gJointDescKeys[cKinTree::eJointDescMax] = {
		"Type",					"Parent",		"AttachX",			 "AttachY",		 "AttachZ",	"AttachThetaX", "AttachThetaY",
		"AttachThetaZ", "LimLow0",	"LimLow1",			 "LimLow2",		 "LimHigh0", "LimHigh1",		 "LimHigh2",
		"TorqueLim",		"ForceLim", "IsEndEffector", "DiffWeight", "Offset"};
const std::string gBodyDefsKey = "BodyDefs";
const std::string gBodyDescKeys[cKinTree::eBodyParamMax] = {
		"Shape",				"Mass",					"ColGroup", "EnableFallContact",
...
void cKinTree::FindChildren(const Eigen::MatrixXd &joint_desc, int joint_id, Eigen::VectorXi &out_children) {
	...
	for (int i = 0; i < num_children; ++i) { out_children[i] = children_buffer[i]; }
}
...
bool cKinTree::ParseBodyDef(const Json::Value &root, cKinTree::tBodyDef &out_def) {
	...
	if (succ) { out_def(eBodyParamShape) = static_cast<double>(static_cast<int>(shape)); }
...
bool cKinTree::ParseDrawShapeDef(const Json::Value &root, tDrawShapeDef &out_def) {
	...
	eBodyShape shape;
	bool succ = ParseBodyShape(shape_str, shape);
	if (succ) { out_def(eDrawShapeShape) = static_cast<double>(static_cast<int>(shape)); }
...
tVector cKinTree::CalcBodyPartPos(const Eigen::MatrixXd &joint_mat, const Eigen::MatrixXd &body_defs,
																	const Eigen::VectorXd &state, int part_id) {
...
/*tVector cKinTree::CalcBodyPartVel(const Eigen::MatrixXd& joint_mat, const Eigen::MatrixXd& body_defs, const
Eigen::VectorXd& state, const Eigen::VectorXd& vel, int part_id)
{
...
tVector cKinTree::GetBodyAttachTheta(const Eigen::MatrixXd &body_defs, int part_id) {
	tVector attach_theta = tVector(body_defs(part_id, eBodyParamAttachThetaX), body_defs(part_id, eBodyParamAttachThetaY),
...
bool cKinTree::IsValidBody(const Eigen::MatrixXd &body_defs, int part_id) {
	...
	if (shape < eBodyShapeMax) { return true; }
	return false;
}
tVector cKinTree::GetBodyLocalCoM(const Eigen::MatrixXd &body_defs, int part_id) {
	...
	switch (shape) {
	case eBodyShapeBox: break;
	case eBodyShapeCapsule: break;
	default:
...
tVector cKinTree::GetDrawShapeColor(const tDrawShapeDef &shape) {
	return tVector(shape[eDrawShapeColorR], shape[eDrawShapeColorG], shape[eDrawShapeColorB],
								 shape[cKinTree::eDrawShapeColorA]);
}
void cKinTree::CalcBodyPartRotation(const Eigen::MatrixXd &joint_mat, const Eigen::MatrixXd &body_defs,
																		const Eigen::VectorXd &state, int part_id, tVector &out_axis, double &out_theta) {
	tMatrix mat = BodyWorldTrans(joint_mat, body_defs, state, part_id);
	cMathUtil::RotMatToAxisAngle(mat, out_axis, out_theta);
}
bool cKinTree::Load(const std::string &char_file, Eigen::MatrixXd &out_joint_mat) {
	bool succ = true;
	std::string str;
	std::ifstream f_stream(char_file.c_str());
	Json::Value root;
  ...
  succ = Json::parseFromStream(reader, f_stream, &root, &str);
	f_stream.close();
	if (succ && !root["Skeleton"].isNull()) { return Load(root["Skeleton"],out_joint_mat); }
	return succ;
}
...
tVector cKinTree::LocalToWorldPos(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &state, int parent_id,
...
/*tVector cKinTree::CalcJointWorldVel(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &state,
																		const Eigen::VectorXd &vel, int joint_id) {
	return CalcWorldVel(joint_mat, state, vel, joint_id, tVector::Zero());
}
tVector cKinTree::CalcWorldVel(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &state,
															 const Eigen::VectorXd &vel, int parent_id, const tVector &attach_pt) {
...
tVector cKinTree::CalcJointWorldAngularVel(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &state,
																					 const Eigen::VectorXd &vel, int joint_id) {
	return CalcWorldAngularVel(joint_mat, state, vel, joint_id, tVector::Zero());
}
tVector cKinTree::CalcWorldAngularVel(const Eigen::MatrixXd& joint_mat, const Eigen::VectorXd& state, const
Eigen::VectorXd& vel, int parent_id, const tVector& attach_pt)
{
	cSpAlg::tSpVec sv = cRBDUtil::CalcWorldVel(joint_mat, state, vel, parent_id);
	tVector pos = cKinTree::LocalToWorldPos(joint_mat, state, parent_id, attach_pt);
	cSpAlg::tSpTrans world_to_pt = cSpAlg::BuildTrans(pos);
...
*/
int cKinTree::GetNumJoints(const Eigen::MatrixXd &joint_mat) { return static_cast<int>(joint_mat.rows()); }
...
Eigen::VectorXi cKinTree::FindJointChain(const Eigen::MatrixXd &joint_mat, int joint_beg, int joint_end) {
	...
	if (found) {
		...
		for (int i = 0; i < buffer_idx; ++i) { chain[i] = chain_buffer[i]; }
		int idx = buffer_idx;
		curr_id = joint_end;
...
void cKinTree::CalcAABB(const Eigen::MatrixXd &joint_desc, const Eigen::VectorXd &state, tVector &out_min,
												tVector &out_max) {
...
int cKinTree::GetJointParamSize(eJointType joint_type) {
	int size = 0;
	switch (joint_type) {
	case eJointTypeRevolute: size = 1; break;
	case eJointTypePrismatic: size = 1; break;
	case eJointTypePlanar: size = 3; break;
	case eJointTypeFixed: size = 0; break;
	case eJointTypeSpherical: size = 4; break;
	default:
...
void cKinTree::GetJointParams(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &state, int j,
...
void cKinTree::SetJointParams(const Eigen::MatrixXd &joint_mat, int j, const Eigen::VectorXd &params,
...
bool cKinTree::IsRoot(const Eigen::MatrixXd &joint_mat, int joint_id) { return !HasParent(joint_mat, joint_id); }
bool cKinTree::IsJointActuated(const Eigen::MatrixXd &joint_mat, int joint_id) { return !IsRoot(joint_mat, joint_id); }
...
tVector cKinTree::GetAttachPt(const Eigen::MatrixXd &joint_mat, int joint_id) {
	tVector attach_pt =
			tVector(joint_mat(joint_id, cKinTree::eJointDescAttachX), joint_mat(joint_id, cKinTree::eJointDescAttachY),
...
void cKinTree::CalcMaxSubChainLengths(const Eigen::MatrixXd &joint_mat, Eigen::VectorXd &out_lengths) {
	...
	for (int j = num_joints - 1; j >= 0; --j) {
		...
		if (parent_id != gInvalidJointID) {
			...
			if (parent_val < len + curr_val) { parent_val = len + curr_val; }
		}
	}
}
void cKinTree::CalcSubTreeMasses(const Eigen::MatrixXd &joint_mat, const Eigen::MatrixXd &body_defs,
																 Eigen::VectorXd &out_masses) {
	int num_joints = static_cast<int>(joint_mat.rows());
...
bool cKinTree::ParseJoint(const Json::Value &root, tJointDesc &out_joint_desc) {
	...
	for (int i = 0; i < eJointDescMax; ++i) {
		if (i != eJointDescType) {
			...
			if (!root[key].isNull()) { out_joint_desc[i] = root[key].asDouble(); }
		}
	}
	return true;
}
...
tMatrix cKinTree::ChildParentTrans(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &state, int joint_id) {
	...
	if (is_root) {
		...
		switch (j_type) {
		case eJointTypeRevolute: mat = ChildParentTransRevolute(joint_mat, state, joint_id); break;
		case eJointTypePrismatic: mat = ChildParentTransPrismatic(joint_mat, state, joint_id); break;
		case eJointTypePlanar: mat = ChildParentTransPlanar(joint_mat, state, joint_id); break;
		case eJointTypeFixed: mat = ChildParentTransFixed(joint_mat, state, joint_id); break;
		case eJointTypeSpherical: mat = ChildParentTransSpherical(joint_mat, state, joint_id); break;
		default: break;
		}
	}
	return mat;
...
tMatrix cKinTree::BodyWorldTrans(const Eigen::MatrixXd &joint_mat, const Eigen::MatrixXd &body_defs,
																 const Eigen::VectorXd &state, int part_id) {
...
void cKinTree::BuildDefaultPose(const Eigen::MatrixXd &joint_mat, Eigen::VectorXd &out_pose) {
	...
	for (int j = 1; j < num_joints; ++j) {
		...
		switch (joint_type) {
		case eJointTypeRevolute: BuildDefaultPoseRevolute(joint_pose); break;
		case eJointTypePrismatic: BuildDefaultPosePrismatic(joint_pose); break;
		case eJointTypePlanar: BuildDefaultPosePlanar(joint_pose); break;
		case eJointTypeFixed: BuildDefaultPoseFixed(joint_pose); break;
		case eJointTypeSpherical: BuildDefaultPoseSpherical(joint_pose); break;
		default:
			assert(false); // unsupported joint type
			break;
		}
...
void cKinTree::BuildDefaultVel(const Eigen::MatrixXd &joint_mat, Eigen::VectorXd &out_vel) {
	...
	for (int j = 1; j < num_joints; ++j) {
		...
		Eigen::VectorXd joint_pose;
		switch (joint_type) {
		case eJointTypeRevolute: BuildDefaultVelRevolute(joint_pose); break;
		case eJointTypePrismatic: BuildDefaultVelPrismatic(joint_pose); break;
		case eJointTypePlanar: BuildDefaultVelPlanar(joint_pose); break;
		case eJointTypeFixed: BuildDefaultVelFixed(joint_pose); break;
		case eJointTypeSpherical: BuildDefaultVelSpherical(joint_pose); break;
		default:
			assert(false); // unsupported joint type
			break;
		}
...
void cKinTree::CalcPoseDiff(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &pose0,
														const Eigen::VectorXd &pose1, Eigen::VectorXd &out_diff) {
	int num_joints = GetNumJoints(joint_mat);
...
tVector cKinTree::CalcRootPosDiff(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &pose0,
...
tQuaternion cKinTree::CalcRootRotDiff(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &pose0,
																			const Eigen::VectorXd &pose1) {
...
double cKinTree::CalcPoseErr(const Eigen::MatrixXd &joint_mat, int joint_id, const Eigen::VectorXd &pose0,
														 const Eigen::VectorXd &pose1) {
	bool is_root = IsRoot(joint_mat, joint_id);
	double err = 0;
	if (is_root) {
		...
		default: err = diff.squaredNorm(); break;
		}
	}
	return err;
}
double cKinTree::CalcRootPosErr(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &pose0,
																const Eigen::VectorXd &pose1) {
	tVector diff = CalcRootPosDiff(joint_mat, pose0, pose1);
	return diff.squaredNorm();
}
double cKinTree::CalcRootRotErr(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &pose0,
																const Eigen::VectorXd &pose1) {
	tQuaternion diff = CalcRootRotDiff(joint_mat, pose0, pose1);
	double theta = cMathUtil::QuatTheta(diff);
	return theta * theta;
}
void cKinTree::CalcVelDiff(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &vel0, const Eigen::VectorXd &vel1,
...
tVector cKinTree::CalcRootVelDiff(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &pose0,
																	const Eigen::VectorXd &pose1) {
	tVector root_vel0 = GetRootVel(joint_mat, pose0);
	tVector root_vel1 = GetRootVel(joint_mat, pose1);
	return root_vel1 - root_vel0;
}
tVector cKinTree::CalcRootAngVelDiff(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &pose0,
																		 const Eigen::VectorXd &pose1) {
...
double cKinTree::CalcVelErr(const Eigen::MatrixXd &joint_mat, int joint_id, const Eigen::VectorXd &vel0,
...
double cKinTree::CalcRootVelErr(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &vel0,
																const Eigen::VectorXd &vel1) {
	tVector diff = CalcRootVelDiff(joint_mat, vel0, vel1);
	return diff.squaredNorm();
}
double cKinTree::CalcRootAngVelErr(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &vel0,
																	 const Eigen::VectorXd &vel1) {
	tVector diff = CalcRootAngVelDiff(joint_mat, vel0, vel1);
	return diff.squaredNorm();
}
void cKinTree::CalcJointPoseDiff(const Eigen::MatrixXd &joint_mat, int joint_id, const Eigen::VectorXd &pose0,
																 const Eigen::VectorXd &pose1, Eigen::VectorXd &out_diff) {
	...
	if (is_root) {
		...
		default: out_diff = pose1.segment(param_offset, param_size) - pose0.segment(param_offset, param_size); break;
		}
	}
}
void cKinTree::CalcJointVelDiff(const Eigen::MatrixXd &joint_mat, int joint_id, const Eigen::VectorXd &vel0,
																const Eigen::VectorXd &vel1, Eigen::VectorXd &out_diff) {
	int param_offset = GetParamOffset(joint_mat, joint_id);
	int param_size = GetParamSize(joint_mat, joint_id);
	out_diff = vel1.segment(param_offset, param_size) - vel0.segment(param_offset, param_size);
}
void cKinTree::CalcVel(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &pose0, const Eigen::VectorXd &pose1,
											 double dt, Eigen::VectorXd &out_vel) {
	...
	for (int j = 1; j < num_joints; ++j) {
		...
		default:
			out_vel.segment(param_offset, param_size) =
					(pose1.segment(param_offset, param_size) - pose0.segment(param_offset, param_size)) / dt;
			break;
		}
	}
}
...
void cKinTree::LerpPoses(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &pose0, const Eigen::VectorXd &pose1,
												 double lerp, Eigen::VectorXd &out_pose) {
	int num_joints = GetNumJoints(joint_mat);
	int root_id = GetRoot(joint_mat);
	int root_offset = GetParamOffset(joint_mat, root_id);
...
void cKinTree::VelToPoseDiff(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &pose, const Eigen::VectorXd &vel,
														 Eigen::VectorXd &out_pose_diff) {
	out_pose_diff = vel;
	int num_joints = GetNumJoints(joint_mat);
	int root_id = GetRoot(joint_mat);
	int root_offset = GetParamOffset(joint_mat, root_id);
...
void cKinTree::MirrorPoseStance(const Eigen::MatrixXd &joint_mat, const std::vector<int> mirror_joints0,
																const std::vector<int> mirror_joints1, Eigen::VectorXd &out_pose) {
...
std::string cKinTree::BuildJointJson(const tJointDesc &joint_desc) {
	...
	for (int i = 0; i < eJointDescMax; ++i) {
		...
		if (i == eJointDescType) {
			...
			if (!std::isfinite(val) && (i == eJointDescTorqueLim || i == eJointDescForceLim)) { continue; }
			if (i != 0) { json += ",\n"; }
...
tMatrix cKinTree::ChildParentTransRevolute(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &state,
																					 int joint_id) {
...
tMatrix cKinTree::ChildParentTransPrismatic(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &state,
																						int joint_id) {
	int param_offset = cKinTree::GetParamOffset(joint_mat, joint_id);
	tVector offset = tVector::Zero();
	offset[0] = state(param_offset);
...
tMatrix cKinTree::ChildParentTransSpherical(const Eigen::MatrixXd &joint_mat, const Eigen::VectorXd &state,
																						int joint_id) {
	int param_offset = cKinTree::GetParamOffset(joint_mat, joint_id);
