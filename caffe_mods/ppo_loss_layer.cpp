...
void PPOLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype> *> &bottom,
	...
	/*caffe_mul(count, this->diff_.cpu_data(), bottom[2]->cpu_data(),
	weighted_diff_.mutable_cpu_data());

	Dtype dot = caffe_cpu_dot(count, this->diff_.cpu_data(),
	weighted_diff_.cpu_data());*/
	...
	int nan_count = 0;
	// printf("step in[%f,(%f,%f)]%f=%f\n",scale,low,high,M_2_PI,M_PI_2);
	// printf("top diff %f\n", top[0]->cpu_diff()[0]);
	// printf("bottom diff %f\n", bottom[0]->cpu_diff()[0]);
	// caffe_sqr(count, this->diff_.cpu_data(),
	// this->advantage.mutable_cpu_data());
#if 1
	{
		caffe_scal(count, Dtype(1 / scale), this->diff_.mutable_cpu_data());
		// caffe_copy(count, this->diff_.cpu_data(),
		// this->newlogp.mutable_cpu_data());  caffe_scal(count, Dtype(1 / scale),
		// this->newlogp.mutable_cpu_data());
		caffe_sqr(count, this->diff_.cpu_data(), this->newlogp.mutable_cpu_data());
		/*		top[0]->mutable_cpu_data()[0] =
						caffe_cpu_asum(count, this->newlogp.mutable_cpu_data()) / count;
		*/
	...
	if (sw == 0.0) {
		...
		sw = Dtype(1.0);
		DEBUG_PPO(printf("first step\n"));
	}
	caffe_scal(count, Dtype(-sqrt(0.5 * M_1_PI)), this->td.mutable_cpu_data());
...
#else
	{
		caffe_copy(count, this->diff_.cpu_data(), this->newlogp.mutable_cpu_data());
		caffe_scal(count, Dtype(1 / scale), this->newlogp.mutable_cpu_data());
		caffe_sqr(count, this->newlogp.cpu_data(),
							this->newlogp.mutable_cpu_data());
		caffe_scal(count, Dtype(-0.5), this->newlogp.mutable_cpu_data());
		caffe_exp(count, this->newlogp.cpu_data(), this->td.mutable_cpu_data());
		caffe_add_scalar(count, -Dtype(0.5 * log(M_PI + M_PI) + log(scale)),
										 this->newlogp.mutable_cpu_data());
	}
	if (sw == 0.0) {
		caffe_copy(count, this->newlogp.cpu_data(),
	...
	// -sqrt(0.5/pi)x*e^(-0.5(x/s)^2)/(s^3);
	caffe_scal(count, Dtype(-sqrt(0.5 * M_1_PI)), this->td.mutable_cpu_data());
	caffe_mul(count, this->td.cpu_data(), this->diff_.cpu_data(),
						this->td.mutable_cpu_data());
	caffe_scal(count, Dtype(1.0 / (scale * scale * scale * bottom[0]->count(1))),
						 this->td.mutable_cpu_data());
	// printf("td %f, %f\n", this->td.cpu_data()[0], sqrt(0.5*M_1_PI));
#endif
