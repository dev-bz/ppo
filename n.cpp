...
void Net::LoadTrainData(const std::vector<float> &X, const std::vector<float> &Y, const std::vector<float> &W) {
	int blobs = tlocal->input_blobs().size();
	{
		auto dt = X.data();
		int jc = X.size();
...
void Net::makeNet(int &i, int &o, const char *net, const char *solver) {
	...
	if (solver) {
		...
		switch (type) {
		case caffe::SolverParameter_SolverType_SGD: slocal.reset(new caffe::SGDSolver<float>(param)); break;
		case caffe::SolverParameter_SolverType_NESTEROV: slocal.reset(new caffe::NesterovSolver<float>(param)); break;
		case caffe::SolverParameter_SolverType_ADAGRAD: slocal.reset(new caffe::AdaGradSolver<float>(param)); break;
		case caffe::SolverParameter_SolverType_RMSPROP: slocal.reset(new caffe::RMSPropSolver<float>(param)); break;
		case caffe::SolverParameter_SolverType_ADADELTA: slocal.reset(new caffe::AdaDeltaSolver<float>(param)); break;
		case caffe::SolverParameter_SolverType_ADAM: slocal.reset(new caffe::AdamSolver<float>(param)); break;
...
float Net::getValues(const std::vector<float> &input, std::vector<float> &output, int batchSize) {
...
void Net::Load(const std::string &model_file) {
	if (model_file != "") {
		if (local != nullptr) {
			if (model_file.size() >= 4 && model_file.compare(model_file.size() - 4, 4, ".bin") == 0) {
...
void Net::check(float weight_decay) {
	if (local != nullptr) {
		...
		printf("check: %f, %f (%d, %d)/%d\n", max, max_diff, nan_count, nan_diff_count, nan_all_count);
...
extern "C" void testPPO() {
	...
	if (solver) {
		...
		std::shared_ptr<caffe::Net<float>> local(new caffe::Net<float>(param.net().c_str(), caffe::TEST));
		int blobs = tlocal->input_blobs().size();
		...
		if (local->input_blobs().size() > 0) {
			...
			for (int j = 0; j < ct; ++j) td[j] = state[j % state_];
		}
		if (blobs > 0) {
			auto state_ = state.size();
			auto &i = tlocal->input_blobs()[0];
